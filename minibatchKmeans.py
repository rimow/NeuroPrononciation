...
def minibatchkmeans(X, n_clusters, path_classement, path_aligned, type_resultats, type_separation, initialisationCentres):
    '''
    ...
    :param type_separation : 0=Consonnes/voyelles, 1=voises/non voises, 2=fricatives,occlusives,etc
    :param initialisationCentres: True ou False
    :return: Affiche les pourcentages
    '''

    n_samples, n_features = X.shape
    period = np.loadtxt(path_aligned,delimiter=' ',usecols=(0,1))
    phoneme = np.loadtxt(path_aligned,dtype= str ,delimiter=' ',usecols=[2])
    ...
    try:
        if initialisationCentres:
            sous = initialisation_centres(n_clusters, X)
            ...
            kmeans = MiniBatchKMeans(n_clusters = n_clusters, init='k-means++',  batch_size=700,
                                  n_init=10, max_no_improvement=10, verbose=0)
        ...
        if type_resultats==0:
            pourcentage(Y , n_clusters, labels , path_classement , type_separation)
        if type_resultats==1:
